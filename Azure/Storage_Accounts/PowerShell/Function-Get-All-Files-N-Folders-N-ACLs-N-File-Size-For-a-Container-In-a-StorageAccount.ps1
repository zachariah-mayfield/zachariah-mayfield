...
#region Main
        ...
        ForEach ($R in $Container_Response.paths) {
            ...
            $Service_Principal | add-member Noteproperty Container_Name $Container -Force
            $Service_Principal | add-member Noteproperty Container_FileSytemName_Owner $R.owner -Force
            $Service_Principal | add-member Noteproperty Container_FileSytemName_Owner $R.owner -Force
            $Service_Principal | add-member Noteproperty Container_FileSytemName_Permissions $R.Permissions -Force
        
            $Service_Principal | add-member Noteproperty FullName_Path ('/' + $R.Name) -Force
            $Service_Principal | add-member Noteproperty Content_Length $R.contentLength -Force
            $Action = 'getAccessControl'
            $ACL_Response = (Invoke-WebRequest -Method Head -Uri "https://$($Storage_Account_Name).$($dnsSuffix)/$($Container)/$($R.Name)?action=$($Action)&upn=$($true)" -Headers $HEADERS)
            $CurrentAcl = $ACL_Response.Headers["x-ms-acl"]
...
Get-Azure_StorageAccountContainer_Targeted_Blobs_N_Folders -Storage_Account_Name 'Storage_Account_Name' -Container 'Container' 
