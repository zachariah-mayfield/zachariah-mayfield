...
IF ($Environment -eq 'Development') {
    ...
    $Certificate_Thumbprint = (Get-ChildItem -Path 'cert:\LocalMachine\my\' | Where-Object {$_.Subject -match "CN=xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx"}).Thumbprint
...
$Entity_Ids = (Get-AzDataLakeGen2Item -Context $Azure_Storage_Context -FileSystem $Container).acl | Where-Object {$null -ne $_.EntityId -and $_.DefaultScope -eq $false}

$Owner = (Get-AzDataLakeGen2Item -Context $Azure_Storage_Context -FileSystem $Container)
...
Else {
    Write-Host -ForegroundColor Cyan "The owner of the Container is: $($Owner.Owner)"
}
