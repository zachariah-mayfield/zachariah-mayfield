...
Function Add-O365ContactToDistributionGroup {
    ...
    Param (
    [String]$CSVLocation = "C:\xxxxx.csv",
...
ForEach ($V in $Values) {
    ...
    IF ($CSV[$v] -ne $null) {
        ...
        ForEach ($Email in $ExternalEmailAddress) {
            ...
            If (Get-MailContact -Anr $Output.Contacts) {
            ...
            If ($CheckDG -eq $null) {
                ...
                $Email = ($Output.DistroGroupName + "@xxx.com")# ""
            ...
            IF ($CheckDGM -match $Output.Contacts){
                ...
                Add-DistributionGroupMember -Identity $Output.DistroGroupName -Member $Output.Contacts -BypassSecurityGroupManagerCheck -Verbose
            }
