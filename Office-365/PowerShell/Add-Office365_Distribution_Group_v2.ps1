...
Try {
    ...
    # Current Oracle List
    $Oraclexxxxx = (Get-OracleQuery | select EMAIL -Unique).Email
    ...
    ELSE {
        ...
        Set-DistributionGroup -Identity $Group -ManagedBy @{Add="office365roomadmins@cfacorp.onmicrosoft.com"} -BypassSecurityGroupManagerCheck -ErrorAction SilentlyContinue | Out-Null

}#END TRY

CATCH {
    $ErrorX = Write-Output $_
    $Body = ("The Oracle Managed Distribution Group: 'xxxxx' encountered an ERROR: " + " 
" + ($ErrorX) | Out-String)
    Send-MailMessage -To $To -From $From -SmtpServer $SMTPServer -Subject $Subject -Body $Body
}

TRY {
    ForEach ($xxxxx in $Oraclexxxxx) {    
        IF ($xxxxx -notin $O365_Distribution_List) {
            # Adding the user to the office 365 froup, if not in the Office 365 group
            write-host -ForegroundColor Green $xxxxx
            Add-DistributionGroupMember -Identity $PrimarySmtpAddress -Member $xxxxx -BypassSecurityGroupManagerCheck
        }
        Else {
            write-host -ForegroundColor Cyan $xxxxx
        }
    }
}
CATCH {
