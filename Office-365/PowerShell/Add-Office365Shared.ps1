...
Try {
    ...
    If ($CheckShared.PrimarySmtpAddress -notmatch $Email -and $CheckEmail -eq $true) {
...
Try {
    Add-MailboxPermission -Identity $SharedBox -User "xxxxx.com" -AccessRights ‘FullAccess’ -InheritanceType All -Confirm:$False -ErrorAction Stop -WarningAction SilentlyContinue | Out-Null
}
Catch {
    Write-Output ("Issue Adding Full Access Member `"xxxxx.com`" to the Shared MailBox `"$SharedBox`"")
    Write-Error -Message $_.Exception.Message
}
...
ForEach($Name in $SendAsMember) {
    ...
    IF ($CheckSendAsMember -in $SendAsMember) {
        Write-Output ("The user: $Name is already a Send As Member in the Shared MailBox: $SharedBox")
    }
