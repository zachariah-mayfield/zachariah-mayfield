...
Clear-Content C:\xxxx\SpeedTestScriptPerformance.log
...
try{
    ...
    For ($startTime = Get-Date; ((Get-Date)-$startTime).TotalSeconds -le 15 -and $index -lt 10; $index++) {
        $downStart = Get-Date
        Invoke-WebRequest https://$($downloadPaths[$index]) -OutFile "C:\xxxx\$($downloadPaths[$index])"
    ...
    $logLine | Out-File C:\xxxx\SpeedTestScriptPerformance.log -Append
...
$_avgUpload = 0
try{
    $uploadPaths = Get-ChildItem C:\xxxx\*mb | Sort-Object -Property Length 
...
#ASSERT: Namespace has been created.
...
#ASSERT: WMI-Object Class has been created with desired variables.
...
#ASSERT: Previous $keyvalue has been removed, if it existed.
...
#ASSERT: WMI-Object has been created and successfully populated into the \root\cimv2\SpeedTest
