...
$Organization_URL = "https://xxxxxxxxxxxxx"
$Personal_Azure_Token = "xxxxxxxxxxxxxxxxxxxxxxxxxxx"
...
ForEach ($V in $Values) {
    IF ($null -ne $CSV_Data[$v]) {
        ...
        # https://xxxxxxxxxxx/xxxxxxxxxxx/xxxxxxxxxx/_apis/build/definitions/xxxxxxxxxx?api-version=5.1
        ...
        If ($Result.process.phases.steps.displayName -match "Publish Artifact: xxxxxxxxxxxxx") {
            $Contingency1 = ('displayName1 = Publish Artifact: xxxxxxxxxxx')
        }
        If ($Result.process.phases.steps.displayName -match "Package xxxxxxxxxx") {
            $Contingency2 = ('displayName2 = xxxxxxxxx')
        }
        If ($Result.process.phases.steps.inputs.PathtoPublish -contains '$(Build.ArtifactStagingDirectory)\xxx') {
            $Contingency3 = ('PathtoPublish = $(Build.ArtifactStagingDirectory)\\xx')
        }
        If ($Result.process.phases.steps.inputs.TargetPath -contains '$(xxxxxxxxx)') {
            $Contingency4 = ('TargetPath = $(xxxxxxxxxxxx)')
        }
        If ($Result.process.phases.steps.inputs.ArtifactType -match "FilePath") {
            $Contingency5 = ('ArtifactType = FilePath')
        }
        If ($Result.process.phases.steps.inputs.ArtifactName -match "xx") {
            $Contingency6 = ('ArtifactName = xx')
        }
        If ($Result.process.phases.steps.task.id -match "xxxxxxxxxxxxxxxx") {
            $Contingency7 = ('Task.xxxxx = xxxxxxxx')
        }
        If ($Result.process.phases.steps.task.id -match "xxxxxxxxxxxx") {
            $Contingency8 = ('Task.xxxxxxxxx = xxxxxxxxxx')
        }
        If ($Result.process.phases.steps.inputs.useDfs -match "true") {
            $Contingency9 = ('useDfs = true')
        }
        ...
        "Contingency1", "Contingency2","Contingency3", "Contingency4", "Contingency5", "Contingency6", "Contingency7", "Contingency8", "Contingency9"  | 
            Export-Csv -LiteralPath "C:\Users\xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx.csv" -NoTypeInformation -Encoding utf8 -NoClobber -Append -Force #>
    }
}
