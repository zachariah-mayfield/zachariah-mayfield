...
$Personal_Azure_Token = "Enter you PAT here"
...
$CSV_Path = "C:\Temp\xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx_.csv"
...
ForEach ($V in $Values) {
    ...
    IF ($null -ne $CSV_Data[$v]) { 
        ...
        try {
            ...
            if (($Original_Definition_json.queue.pool.name | Where-Object {$_ -like "Package xxxx *"})) {          
                ...
                $Original_Definition_json.queue.pool | Add-Member -NotePropertyName "id" -NotePropertyValue "1676" -Force    
                $Original_Definition_json.queue.pool | Add-Member -NotePropertyName "name" -NotePropertyValue "xxxx.pool.02" -Force 
                $Original_Definition_json.queue | Add-Member -NotePropertyName "id" -NotePropertyValue "" -Force 
                $Original_Definition_json.queue | Add-Member -NotePropertyName "name" -NotePropertyValue "xxxxx.pool.02" -Force 
            }
            $Specific_Original_Definition_JSON_Task_Id = ($Original_Definition_json.process.phases.steps | Where-Object {$_.task.id -eq "xxxxxxxxxxxxxxxxxxxxxxxxx"})
