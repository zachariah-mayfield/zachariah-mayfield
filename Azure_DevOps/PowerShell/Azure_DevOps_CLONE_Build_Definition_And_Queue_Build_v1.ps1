...
$Personal_Azure_Token = "xxxxxxxxx"
...
$CSV_Path = "C:\Temp\xxxxxxxxxx.csv"
...
ForEach ($V in $Values) {
    ...
    IF ($null -ne $CSV_Data[$v]) { 
        ...
        TRY {
            ...
            try {
                ...
                if (($json.queue.pool.name | Where-Object {$_ -like "Package xx *"})) {          
                    ...
                    $json.queue.pool | Add-Member -NotePropertyName "id" -NotePropertyValue "xxxx" -Force    
                    $json.queue.pool | Add-Member -NotePropertyName "name" -NotePropertyValue "xxxx.02" -Force 
                    $json.queue | Add-Member -NotePropertyName "id" -NotePropertyValue "" -Force 
                    $json.queue | Add-Member -NotePropertyName "name" -NotePropertyValue "xxxxx.02" -Force 
                }

                $Specific_JSON_Task_Id = ($json.process.phases.steps | Where-Object {$_.task.id -eq "xxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxxx"})
        ...
        $CSV_DATA_File = "C:\Temp\Cloned_Builds.csv"
